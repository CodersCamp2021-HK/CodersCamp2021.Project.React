...
describe('tileRules', () => {
  it('matches every possible neighbourhood to exactly one rule', () => {
    ...
    const allPossibleNeighbourhoods = _.times(
      // Number of possible neighbour combinations: (count of tile types)^(count of tiles in neighbourhood)
      TILE_TYPES ** (PATTERN_SIDE ** 2),
      (n) =>
        _.chunk(
